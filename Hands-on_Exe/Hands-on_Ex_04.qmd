...
### Import for exam data.

The same dataset exam data is being used for this exercise
```{r}
exam <-exam <- read_csv("data/Exam_data.csv")
```
...
Code for visualising English scores
...
ecdf referes to the empirica; cumulative distribution. THis is alternatives to visualise distriubution and it could handle bithw continuous and categorical vairables. the downside of this is that it will requires more training to accurately interpret. 

For the could chunk below put the two charts side by side. 
...
```
::: callout note
There are two variables with high collinearity Age_08_04 and Mfg_Year.
:::
